...
if (printplot)
        ...
        %if we have a PC...
        if (comp(1:2) == 'PC')
	  %check if we want a device other than LPT1: 
	  %if so, strip LPT1: from the command and replace it
          printer=deblank(printer(4:length(printer)));
          cmd = sprintf(lprcmd, filename);
	  if (~strcmp(printer,'LPT1') & ~strcmp(printer,'LPT1:')),
            pns=findstr(cmd,'LPT1');
            cmd(pns:pns+3)=printer(1:4);
	  end;
          dos(cmd);
          delete(filename);
